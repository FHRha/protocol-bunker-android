...
import type { ThemeMode, ToastDuration, ToastPosition, UiScale } from "./uiPreferences";

type AppSettingsPopoverLocale = {
  settingsTitle: string;
  settingsGameSectionTitle: string;
  streamerModeLabel: string;
  settingsShowRoomCodeInLobby: string;
  settingsToastPosition: string;
  toastPosTopRight: string;
  toastPosTopLeft: string;
  toastPosBottomRight: string;
  toastPosBottomLeft: string;
  settingsToastDuration: string;
  toastDuration3s: string;
  toastDuration4s: string;
  toastDuration6s: string;
  settingsUiScale: string;
  settingsReduceMotion: string;
  settingsConfirmDangerous: string;
  settingsConfirmExit: string;
  settingsCompactMode: string;
...
interface AppSettingsPopoverProps {
  ...
  locale: AppSettingsPopoverLocale;
  open: boolean;
  setOpen: (value: boolean | ((prev: boolean) => boolean)) => void;
  closeThemeMenu: () => void;
  settingsSectionsCollapsed: { game: boolean; locale: boolean };
  toggleSettingsSection: (section: "game" | "locale") => void;
  isSpectateRoute: boolean;
  ...
  setToastDuration: (value: ToastDuration) => void;
  uiScale: UiScale;
  setUiScale: (value: UiScale) => void;
  reduceMotion: boolean;
  setReduceMotion: (value: boolean) => void;
  confirmDangerousActions: boolean;
  setConfirmDangerousActions: (value: boolean) => void;
  confirmExitGame: boolean;
  setConfirmExitGame: (value: boolean) => void;
  compactMode: boolean;
  setCompactMode: (value: boolean) => void;
  autoCopyRoomCode: boolean;
  setAutoCopyRoomCode: (value: boolean) => void;
  showHints: boolean;
...
export function AppSettingsPopover({
  ...
  return (
    <div className="topbar-popover" ref={popoverRef}>
      ...
      >
        <svg className="topbar-icon-svg" viewBox="0 0 32 32" fill="none" aria-hidden="true">
          <path
      ...
      {open ? (
        <div className="topbar-popover-menu">
          <div className="topbar-popover-title">{locale.settingsTitle}</div>
          <div className="topbar-popover-section">
            ...
            >
              <span className="topbar-popover-section-title">{locale.settingsGameSectionTitle}</span>
              <svg
            ...
            {!settingsSectionsCollapsed.game ? (
              <div className="topbar-popover-section-body">
                {!isSpectateRoute ? (
                  <>
                    <label className="topbar-menu-checkbox">
                      ...
                      />
                      <span>{locale.streamerModeLabel}</span>
                    </label>
                    <label className="topbar-menu-checkbox">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span>{locale.settingsShowRoomCodeInLobby}</span>
                    </label>
                  </>
                ) : null}
                <label className="topbar-menu-field">
                  <span>{locale.settingsToastPosition}</span>
                  <select value={toastPosition} onChange={(event) => setToastPosition(event.target.value as ToastPosition)}>
                    <option value="top-right">{locale.toastPosTopRight}</option>
                    <option value="top-left">{locale.toastPosTopLeft}</option>
                    <option value="bottom-right">{locale.toastPosBottomRight}</option>
                    <option value="bottom-left">{locale.toastPosBottomLeft}</option>
                  </select>
                </label>
                <label className="topbar-menu-field">
                  <span>{locale.settingsToastDuration}</span>
                  <select value={toastDuration} onChange={(event) => setToastDuration(event.target.value as ToastDuration)}>
                    <option value="3000">{locale.toastDuration3s}</option>
                    <option value="4000">{locale.toastDuration4s}</option>
                    <option value="6000">{locale.toastDuration6s}</option>
                  </select>
                </label>
                <label className="topbar-menu-field">
                  <span>{locale.settingsUiScale}</span>
                ...
                <label className="topbar-menu-checkbox">
                  ...
                  <span>{locale.settingsReduceMotion}</span>
                </label>
                <label className="topbar-menu-checkbox">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span>{locale.settingsConfirmDangerous}</span>
                </label>
                {!isSpectateRoute ? (
                  <label className="topbar-menu-checkbox">
                    <input
                ...
                {!isSpectateRoute ? (
                  <label className="topbar-menu-checkbox">
                    ...
                    />
                    <span>{locale.settingsAutoCopyRoomCode}</span>
                  </label>
                ) : null}
                <label className="topbar-menu-checkbox">
          ...
          <div className="topbar-popover-section">
            ...
            >
              <span className="topbar-popover-section-title">{locale.settingsLocaleSectionTitle}</span>
              <svg
