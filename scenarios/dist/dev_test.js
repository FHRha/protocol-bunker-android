...
const DEV_CHOICE_BASE = IMPLEMENTED_SPECIALS.find((item) => item.effect.type === "forceRevealCategoryForAll") ??
    IMPLEMENTED_SPECIALS.find((item) => resolveChoiceKind(item) !== "none");
const buildDevChoiceDefinition = (playerId) => {
    if (!DEV_CHOICE_BASE)
        return null;
...
export const scenario = {
    ...
    createSession(ctx) {
        ...
        const createPlayerState = (playerId, name, isBot) => {
            const hand = [];
        ...
        const removePlayerFromVoting = (targetId) => {
            if (!votingState)
                return;
        ...
        const removeDevPlayer = (targetId) => {
            ...
            if (!idToRemove)
                return { error: "Нет игрока для удаления." };
        ...
        const autoRevealBots = () => {
            ...
            if (!player || !player.isBot || player.status !== "alive")
                return;
            ...
            let candidate = forcedCategory
                ? player.hand.find((card) => card.deck === (deckInfo?.deck ?? forcedCategory) &&
        ...
        const autoVoteBots = () => {
            if (!DEV_AUTO_BOTS || phase !== "voting" || !votingState || votePhase !== "voting")
                return;
            for (const player of players.values()) {
                if (!player.isBot || player.status !== "alive")
                    continue;
        ...
        const validateRequires = (player, special, payload) => {
            ...
            for (const requirement of requires) {
                ...
                if (requirement === "targetHasRevealedSameCategory") {
                    ...
                    const hasRevealed = (DEV_SHOW_ALL_PUBLIC ? getAnyCardsByCategory(neighbor, deckName) : getRevealedCardsByCategory(neighbor, deckName))
                        .length > 0;
                    if (!hasRevealed)
        ...
        const applySpecialEffect = (player, special, payload) => {
            ...
            if (!def.implemented)
                return { error: "Эта карта ещё не реализована." };
            if (special.used && !DEV_ALLOW_REUSE_SPECIAL)
                return { error: "Эта карта уже использована." };
        ...
        const buildPublicCategories = (player) => {
            return CATEGORY_ORDER.map((category) => {
                ...
                const visibleCards = player.hand.filter((card) => {
                    const matchDeck = card.deck === deckInfo.deck && (!deckInfo.slotKey || card.slotKey === deckInfo.slotKey);
                    if (!matchDeck)
                        return false;
