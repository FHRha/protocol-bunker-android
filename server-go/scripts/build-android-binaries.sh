...
if [[ -d "$ROOT_DIR/android-app/src/main/assets/server-binaries" ]]; then
  ASSETS_DIR="$ROOT_DIR/android-app/src/main/assets/server-binaries"
elif [[ -d "$ROOT_DIR/android-app/app/src/main/assets/server-binaries" ]]; then
  ASSETS_DIR="$ROOT_DIR/android-app/app/src/main/assets/server-binaries"
else
  ASSETS_DIR="$ROOT_DIR/android-app/src/main/assets/server-binaries"
fi

echo "Using Android assets dir: $ASSETS_DIR"
...
resolve_sdk_root() {
  ...
  if [[ -d "$ANDROID_APP_DIR/.android-sdk" ]]; then printf '%s' "$ANDROID_APP_DIR/.android-sdk"; return; fi
...
resolve_llvm_bin() {
  ...
  if [[ -z "$ndk_version_dir" ]]; then echo "No NDK versions found in $ndk_root." >&2; exit 1; fi
  local host_tag=""
  case "$(uname -s)" in
    Linux*) host_tag="linux-x86_64" ;;
    Darwin*) if [[ "$(uname -m)" == "arm64" ]]; then host_tag="darwin-arm64"; else host_tag="darwin-x86_64"; fi ;;
    *) echo "Unsupported host OS for NDK toolchain: $(uname -s)" >&2; exit 1 ;;
  esac
  local llvm_bin="$ndk_root/$ndk_version_dir/toolchains/llvm/prebuilt/$host_tag/bin"
  if [[ ! -d "$llvm_bin" ]]; then echo "NDK LLVM toolchain not found: $llvm_bin" >&2; exit 1; fi
  printf '%s' "$llvm_bin"
}

build_one() {
  local goarch="$1" abi="$2" clang_target="$3" llvm_bin="$4" goarm="${5:-}"
  local out="$ASSETS_DIR/$abi/server-go"
  mkdir -p "$(dirname "$out")"
  echo "Building $abi -> $out"
  local cc="$llvm_bin/$clang_target"
  if [[ ! -x "$cc" ]]; then echo "Clang target wrapper not found for $abi: $cc" >&2; exit 1; fi
  if [[ -n "$goarm" ]]; then
    (cd "$SERVER_DIR" && GOOS=android GOARCH="$goarch" GOARM="$goarm" CGO_ENABLED=1 CC="$cc" go build -trimpath -ldflags='-s -w' -o "$out" .)
  else
    (cd "$SERVER_DIR" && GOOS=android GOARCH="$goarch" CGO_ENABLED=1 CC="$cc" go build -trimpath -ldflags='-s -w' -o "$out" .)
  fi
  chmod +x "$out"
}
