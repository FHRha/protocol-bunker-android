...
export const computeTargetScope = (uiTargeting, text) => {
    const targeting = normalize(uiTargeting);
    const body = `${targeting} ${normalize(text)}`.trim();
    if (!body)
        return null;
    if (includesAny(body, ["no target", "passive", "auto", "без цели", "пассивн"]))
        return null;
    if (includesAny(body, [
        "neighbor",
        "сосед",
...
export const computeNeighbors = (orderRing, aliveSet, actorId) => {
...
export const getTargetCandidates = (scope, actorId, orderRing, aliveSet) => {
    if (scope === "self") {
    ...
    if (scope === "neighbors") {
        ...
        if (neighbors.leftId && neighbors.leftId !== actorId)
            candidates.add(neighbors.leftId);
        if (neighbors.rightId && neighbors.rightId !== actorId)
            candidates.add(neighbors.rightId);
